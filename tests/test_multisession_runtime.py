...
class FeishuTopicSessionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.old_default_llms = fsapp.chat_default_llms
    ...
    def tearDown(self):
        ...
        fsapp.chat_default_llms = self.old_default_llms
...
class FeishuSessionPersistenceTests(unittest.TestCase):
    ...
    def test_legacy_state_migrates_to_current_schema(self):
        ...
        self.assertEqual(fsapp.chat_default_llms["chat-1"], 2)
...
class _ModelAgent:
    def __init__(self):
        self.llm_no = 0
        self.llmclient = object()
        self.history = []

    def next_llm(self, n=-1):
        self.llm_no = ((self.llm_no + 1) if n < 0 else n) % 4

    def list_llms(self):
        return [(idx, f"model-{idx}", idx == self.llm_no) for idx in range(4)]
...
class _ModelRuntime:
    ...
    def get_or_create(self, session_id, metadata=None):
        if session_id not in self.sessions:
            self.sessions[session_id] = _ModelSession(session_id, metadata=metadata)
        elif metadata:
...
class FeishuModelSwitchTests(unittest.TestCase):
    def setUp(self):
        import frontends.fsapp as fsapp
        ...
        self.old_default_llms = fsapp.chat_default_llms
        self.old_persisted = fsapp._persisted_sessions
        self.old_send_message = fsapp.send_message
        fsapp.SESSION_STATE_FILE = self.state_file
        fsapp.runtime = _ModelRuntime()
        fsapp.session_aliases = {}
        fsapp.chat_active_sessions = {}
        fsapp.chat_default_llms = {}
        fsapp._persisted_sessions = {}
        fsapp.send_message = lambda *args, **kwargs: self.sent.append((args, kwargs))
    ...
    def tearDown(self):
        ...
        fsapp.chat_default_llms = self.old_default_llms
        fsapp._persisted_sessions = self.old_persisted
        fsapp.send_message = self.old_send_message
        self.tmp.cleanup()

    def _message(self, message_id):
        return SimpleNamespace(message_id=message_id, root_id=None, parent_id=None, thread_id=None)

    def test_llm_command_sets_chat_default_for_future_direct_topics(self):
        fsapp = self.fsapp
        first, _ = fsapp.resolve_feishu_session("ou-1", "chat-1", self._message("m1"))

        fsapp.handle_command("ou-1", "/llm 3", chat_id="chat-1", session=first)
        second, _ = fsapp.resolve_feishu_session("ou-1", "chat-1", self._message("m2"))

        self.assertEqual(first.agent.llm_no, 3)
        self.assertEqual(fsapp.chat_default_llms["chat-1"], 3)
        self.assertEqual(second.agent.llm_no, 3)
        self.assertEqual(second.metadata["llm_no"], 3)

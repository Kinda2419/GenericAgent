...
class MultiSessionRuntimeTests(unittest.TestCase):
    ...
    def test_retryable_llm_text_failure_switches_llm_before_done(self):
        agent = GeneraticAgent()
        agent.verbose = False
        agent.llmclients = [
            SimpleNamespace(backend=SimpleNamespace(history=[], name="bad", model="bad-model"), last_tools=""),
            SimpleNamespace(backend=SimpleNamespace(history=[], name="good", model="good-model"), last_tools=""),
        ]
        agent.llmclient = agent.llmclients[0]
        agent.llm_no = 0
        agent._default_llm_applied = True

        calls = []
        original_runner = agentmain.agent_runner_loop
        original_prompt = agentmain.get_system_prompt

        def fake_runner(client, *args, **kwargs):
            calls.append(client.backend.name)
            if client.backend.name == "bad":
                yield "!!!Error: HTTP 502: Upstream service temporarily unavailable"
            else:
                yield "provider recovered"
            return {"result": "CURRENT_TASK_DONE", "data": None}

        try:
            agentmain.agent_runner_loop = fake_runner
            agentmain.get_system_prompt = lambda: ""
            display_queue = __import__("queue").Queue()

            agent._run_task_with_repair("task", "test", [], display_queue)

            items = []
            while not display_queue.empty():
                items.append(display_queue.get_nowait())
            self.assertEqual(calls, ["bad", "good"])
            self.assertEqual(items[-1]["done"], "provider recovered")
            self.assertTrue(any("retryable provider error" in item.get("next", "") for item in items))
        finally:
            agentmain.agent_runner_loop = original_runner
            agentmain.get_system_prompt = original_prompt

    def test_normal_answer_mentioning_http_status_does_not_retry(self):
        agent = GeneraticAgent()
        agent.verbose = False
        agent.llmclients = [
            SimpleNamespace(backend=SimpleNamespace(history=[], name="main", model="main-model"), last_tools=""),
            SimpleNamespace(backend=SimpleNamespace(history=[], name="fallback", model="fallback-model"), last_tools=""),
        ]
        agent.llmclient = agent.llmclients[0]
        agent.llm_no = 0
        ...
        def fake_runner(client, *args, **kwargs):
            calls.append(client.backend.name)
            yield "HTTP 429 means the provider is rate limiting requests."
            return {"result": "CURRENT_TASK_DONE", "data": None}
        ...
        try:
            ...
            agent._run_task_with_repair("explain HTTP 429", "test", [], display_queue)

            items = []
            while not display_queue.empty():
                items.append(display_queue.get_nowait())
            self.assertEqual(calls, ["main"])
            self.assertEqual(items[-1]["done"], "HTTP 429 means the provider is rate limiting requests.")
        finally:
            agentmain.agent_runner_loop = original_runner
            agentmain.get_system_prompt = original_prompt

    def test_run_always_marks_task_done_when_setup_fails(self):
        agent = GeneraticAgent()
        original_get_system_prompt = agentmain.get_system_prompt
        try:
            agentmain.get_system_prompt = lambda: (_ for _ in ()).throw(RuntimeError("prompt failed"))
            dq = agent.put_task("task", source="test")
            worker = threading.Thread(target=agent.run, daemon=True)
            worker.start()
            done = dq.get(timeout=3)
            agent.task_queue.join()

            self.assertIn("prompt failed", done["done"])
            self.assertFalse(agent.is_running)
            self.assertEqual(agent.task_queue.unfinished_tasks, 0)
        finally:
            agentmain.get_system_prompt = original_get_system_prompt
...
class FeishuTopicSessionTests(unittest.TestCase):
    ...
    def test_short_followup_reuses_active_chat_session_without_thread_alias(self):
        original, _ = self.fsapp.resolve_feishu_session("ou", "chat", self._message("m1"))
        self.fsapp.chat_active_sessions["chat"] = original.session_id

        followup, is_new = self.fsapp.resolve_feishu_session(
            "ou", "chat", self._message("m2"), user_input="重试"
        )

        self.assertFalse(is_new)
        self.assertIs(original, followup)
        self.assertEqual(self.fsapp.session_aliases["chat:m2"], original.session_id)
...
class FeishuSessionPersistenceTests(unittest.TestCase):
    ...
    def test_persist_writes_schema_version_and_survives_reload(self):
        ...
        session.agent.llmclient.backend.history = [
            {"role": "user", "content": [{"type": "text", "text": "previous task"}]},
        ...
        self.assertEqual(len(restored.agent.llmclient.backend.history), 2)
        self.assertEqual(restored.agent.llmclient.backend.history[0]["content"][0]["text"], "previous task")

    def test_restart_followup_restores_active_session_llm_history(self):
        fsapp = self.fsapp
        session, _ = fsapp.resolve_feishu_session("ou-1", "chat-1", self._message("m1"))
        session.agent.history.append("[USER]: previous task")
        session.agent.llmclient.backend.history = [
            {"role": "user", "content": [{"type": "text", "text": "previous task"}]},
            {"role": "assistant", "content": [{"type": "text", "text": "working result"}]},
        ]
        fsapp._persist_feishu_sessions()
        ...
        restored, is_new = fsapp.resolve_feishu_session(
            "ou-1", "chat-1", self._message("m2"), user_input="重试"
        )

        self.assertFalse(is_new)
        self.assertEqual(restored.session_id, "chat-1:m1")
        self.assertEqual(len(restored.agent.llmclient.backend.history), 2)
        self.assertEqual(restored.agent.history, ["[USER]: previous task"])
